...
help:
	...
	@echo "  make theta-uvc       [ROBOT]  expose Theta X UVC to /dev/video10 (host)"
	@echo "  make theta-uvc-kill  [ROBOT]  stop the Theta UVC feed (after a camera reboot)"
	@echo "  make theta-stream    [ROBOT]  headless: Theta to Zenoh (view on host)"
	...
	@echo "  make record-frames   [CLIENT] save 360 deg frames to disk (offline analysis)"
...
steps:
	@echo "VAT pre-POC to POC runbook  (router=$(ROUTER_IP))"
	...
	@echo "  [ROBOT]  make theta-uvc        # Theta X to /dev/video10 (leave running)"
	@echo "  [ROBOT]  make theta-stream     # headless: Theta to Zenoh; view via test_frames_server"
